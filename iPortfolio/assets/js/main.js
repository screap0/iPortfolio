...
document.getElementById('FrmContact').addEventListener('submit', function(event) {
    // Formun varsayılan gönderme davranışını önle
    event.preventDefault();

    // Form verilerini topla
    var formData = new FormData(event.target);

    // Formu Fetch API ile gönder
    fetch(event.target.action, {
        method: 'POST',
        body: formData,
    })
    .then(function(response) {
        // Sunucudan yanıt alındığında form alanlarını temizle
        ...
        notification.style.boxShadow = '0px 8px 16px 0px rgba(0,0,0,0.2)';
